...
public class ExcelExporter {
	...
	/**
	 * Creates Excel file from given list of Sections
	 * 
	...
	private static void createSectionsSheet(List<Section> sections, Workbook workbook) {
		...
		int rowNum = 1;
		// Keep check on max GeologicalClasses in any given section and accordingly
		// populate header row later
		int maxGeoClasses = 0;
		for (Section section : sections) {
			...
			if (maxGeoClasses < colNum)
				maxGeoClasses = colNum;
		}
